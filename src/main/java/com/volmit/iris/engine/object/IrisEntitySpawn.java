...
public class IrisEntitySpawn implements IRare {
            ...
            for (int id = 0; id < spawns; id++) {
                ...
                switch (getReferenceSpawner().getGroup()) {
                    case NORMAL:
                        ...
                        break;
                    case CAVE:
                        l = gen.getMantle().findMarkers(c.getX(), c.getZ(), MarkerMatter.CAVE_FLOOR)
                            .convert((i) -> i.toLocation(c.getWorld()).add(0, 1, 0)).getRandom(rng);
                        break;
                    case UNDERWATER: case BEACH:
    ...
    private Entity spawn100(Engine g, Location at) {
        try {
            ...
            if (irisEntity == null || irisEntity.getSurface() == null || !irisEntity.getSurface().matches(at.clone().subtract(0, 1, 0).getBlock()))
